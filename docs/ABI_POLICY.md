...
Release `3.0.0` establishes stable ABI major `3`. Within the supported compiler, standard-library, runtime, architecture, and build-mode family, later `3.x` releases preserve the public binary interfaces documented here. An incompatible public ABI change requires a new major release.
...
The UniGUI, bundled Dear ImGui, and bundled ImPlot shared targets use the package major as their loader identity; for release `3.0.0`, SOVERSION is `3`. On ELF platforms, the core shared library ELF symbol version node is `UNIGUI_3` and remains unchanged throughout compatible `3.x` releases. Windows DLL and import-library names carry the same ABI major. The CMake package version uses `SameMajorVersion`: a newer `3.x` package can satisfy a `3.0` request, while a different major cannot.
